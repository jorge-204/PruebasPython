...
server:
	docker run --rm --volume `pwd`:/opt/calc --name apiserver --network-alias apiserver --env PYTHONPATH=/opt/calc --env FLASK_APP=app/api.py -p 5005:5000 -w /opt/calc calculator-app:latest flask run --host=0.0.0.0
...
test-api:
	...
	docker run -d --rm --volume `pwd`:/opt/calc --network host --env PYTHONPATH=/opt/calc --name apiserver --env FLASK_APP=app/api.py -p 5005:5000 -w /opt/calc calculator-app:latest flask run --host=0.0.0.0
	docker run --rm --volume `pwd`:/opt/calc --network host --env PYTHONPATH=/opt/calc --env BASE_URL=http://apiserver:5000/ -w /opt/calc calculator-app:latest pytest --junit-xml=results/api_result.xml -m api  || true
...
test-e2e:
	...
	docker run --rm --volume `pwd`/test/e2e/cypress.json:/cypress.json --volume `pwd`/test/e2e/cypress:/cypress --volume `pwd`/results:/results  --network host cypress/included:4.9.0 --browser chrome || true
